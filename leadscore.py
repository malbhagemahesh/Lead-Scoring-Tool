...
def run_streamlit_app():
    ...
    # 🔹 Lead Summary
    hot_count = (df['category'] == 'Hot').sum()
    warm_count = (df['category'] == 'Warm').sum()
    cold_count = (df['category'] == 'Cold').sum()

    st.markdown(
        f"### Lead Summary: 🔥 **Hot:** {hot_count} | ⚡ **Warm:** {warm_count} | ❄️ **Cold:** {cold_count}"
    )
